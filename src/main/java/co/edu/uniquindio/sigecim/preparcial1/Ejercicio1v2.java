...
public class Ejercicio1v2 {
    ...
    private boolean[][] visitado; // Matriz para realizar el seguimiento de las celdas visitadas

    /* --------------------- PRUEBA DEL ALGORITMO --------------------- */
    public static void main(String[] args) {
        Ejercicio1v2 m = new Ejercicio1v2();
        int[] totalPresos = {0};  // contador de presos
        m.visitado = new boolean[m.carcel.length][m.carcel[0].length]; // Inicializar la matriz visitada
    ...
    private boolean paso(int x, int y, int[] totalPresos) {
        ...
        if (x < 0 || y < 0 || x >= carcel.length || y >= carcel[0].length || visitado[x][y]) {
            return false;  // No continuamos si la celda ya fue visitada o está fuera de los límites
        }
        ...
        if (carcel[x][y].equals("P")) {
            totalPresos[0]++;
            return false;  // No podemos atravesar celdas "P"
        }

        // Si es un muro, no podemos continuar
        if (carcel[x][y].equals("X")) {
            return false;  // No podemos atravesar celdas "X"
        }

        // Si hemos llegado al punto de control 'G', no detenemos el recorrido aún
        if (carcel[x][y].equals("G")) {
            puntoControl = true;
