...
//    };
    ...
    private boolean paso(int x, int y) {
        ...
        if (laberinto[x][y]=='X'){ // si hemos llegado a X quiere decir que hemos encontrado solución
            imprimirLaberinto(0,0); // imprimimos la solución
            System.out.println();
            return false; // No paramos el algoritmo, buscamos otras soluciones (muchas soluciones)
            // return true; // si hemos encontrado la salida, entonces el algoritmo termina (una solución)
        }
